...
const STORAGE_KEY="@toDos_cooler_noenergy"
...
export default function MakeKit_Cooler_NoEnergy({ navigation, finalhi,setFinalhi, final_select, Setfinal_select,Navi,setNavi }) {
  ...
  function getTentInfo(){
    ...
    get(child(dbRef, "cooler_noenergy/cooler_noenergy")).then((snapshot) => {
  ...
  function getProductInfo(){
    const dbRef = ref(getDatabase());
    get(child(dbRef, "cooler_noenergy/product1")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct1(dbdata);
    });
    get(child(dbRef, "cooler_noenergy/product2")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct2(dbdata);
    });
    get(child(dbRef, "cooler_noenergy/product3")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct3(dbdata);
    });
  }
