...
export default function MakeKit_Table({ navigation, finalhi,setFinalhi, final_select, Setfinal_select,Navi,setNavi }) {
  ...
  function getTentInfo(){
    ...
    get(child(dbRef, "table/table")).then((snapshot) => {
  ...
  function getProductInfo(){
    const dbRef = ref(getDatabase());
    get(child(dbRef, "table/product1")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct1(dbdata);
    });
    get(child(dbRef, "table/product2")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct2(dbdata);
    });
    get(child(dbRef, "table/product3")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct3(dbdata);
    });
  }
