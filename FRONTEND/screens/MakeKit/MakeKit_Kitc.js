...
export default function MakeKit_Kitc({ navigation, finalhi,setFinalhi, final_select, Setfinal_select,Navi,setNavi }) {
  ...
  function getTentInfo(){
    ...
    get(child(dbRef, "kitchen/kitchen")).then((snapshot) => {
  ...
  function getProductInfo(){
    const dbRef = ref(getDatabase());
    get(child(dbRef, "kitchen/product1")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct1(dbdata);
    });
    get(child(dbRef, "kitchen/product2")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct2(dbdata);
    });
    get(child(dbRef, "kitchen/product3")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct3(dbdata);
    });
  }
