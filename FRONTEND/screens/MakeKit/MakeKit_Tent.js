...
export default function MakeKit_Tent({ navigation, finalhi,setFinalhi, final_select, Setfinal_select,Navi,setNavi }) {
  ...
  function getTentInfo(){
    ...
    get(child(dbRef, "tent/tent")).then((snapshot) => {
  ...
  function getProductInfo(){
    const dbRef = ref(getDatabase());
    get(child(dbRef, "tent/product1")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct1(dbdata);
    });
    get(child(dbRef, "tent/product2")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct2(dbdata);
    });
    get(child(dbRef, "tent/product3")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct3(dbdata);
    });
  }
            ...
            <View style={styles.modalView}>
              ...
              <View style={{ height: windowHeight / 1.5, width: windowWidth / 1.1, backgroundColor: "grey" }}>
                <ScrollView horizontal={false}>
                  ...
                  <Image source={require("../../assets/images/Product/T_MAX-01.png")} style={{width: windowWidth/1.1, height: 2500, resizeMode:"contain"}}/>
                </ScrollView>
              </View>
