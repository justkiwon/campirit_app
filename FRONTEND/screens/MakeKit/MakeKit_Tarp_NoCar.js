...
const STORAGE_KEY="@toDos_tarp_nocar"
...
export default function MakeKit_Tarp_NoCar({ navigation, finalhi,setFinalhi, final_select, Setfinal_select,Navi,setNavi }) {
  ...
  function getTentInfo(){
    ...
    get(child(dbRef, "tarp_nocar/tarp_nocar")).then((snapshot) => {
  ...
  function getProductInfo(){
    const dbRef = ref(getDatabase());
    get(child(dbRef, "tarp_nocar/product1")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct1(dbdata);
    });
    get(child(dbRef, "tarp_nocar/product2")).then((snapshot) => {
      var dbdata = snapshot.val();
      setProduct2(dbdata);
    });
  }
